...
class ConfigurationAIB(models.Model):
    libelle = models.CharField(max_length=200)
    taux = models.DecimalField(max_digits=5, decimal_places=2, help_text="Pourcentage, ex. 1.00")
    compte_aib = models.ForeignKey(
        "comptabilite.CompteComptable", on_delete=models.PROTECT, related_name="config_aib_acompte"
    )
    compte_tresorerie = models.ForeignKey(
        "comptabilite.CompteComptable", on_delete=models.PROTECT, related_name="config_aib_tresorerie"
    )
    journal = models.ForeignKey("comptabilite.Journal", on_delete=models.PROTECT, related_name="config_aib")
    actif = models.BooleanField(default=True)
...
class DeclarationAIB(models.Model):
    STATUT_CHOICES = [("BROUILLON", "Brouillon"), ("VALIDEE", "Validée")]

    configuration = models.ForeignKey(ConfigurationAIB, on_delete=models.PROTECT, related_name="declarations")
    annee = models.PositiveSmallIntegerField()
    ...
    montant_aib = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal("0.00"))
    statut = models.CharField(max_length=10, choices=STATUT_CHOICES, default="BROUILLON")
    piece = models.ForeignKey(
        "comptabilite.PieceComptable", on_delete=models.SET_NULL, null=True, blank=True,
        related_name="declarations_aib",
    )
    bordereau = models.FileField(upload_to="bordereaux_aib/", null=True, blank=True)
    date_creation = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"AIB {self.periode_num:02d}/{self.annee} — {self.montant_aib}"
