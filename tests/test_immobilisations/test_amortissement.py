...
from apps.immobilisations.amortissement import coefficient_degressif, plan_degressif
...
def test_degressif_premiere_annuite_superieure_au_lineaire():
    ...
    lignes = plan_degressif(Decimal("100000.00"), Decimal("0.00"), 5, date(2026, 1, 1))
    annuite_2026 = sum((l.montant for l in lignes if l.annee == 2026), Decimal("0.00"))
...
def test_degressif_bascule_en_lineaire_en_fin_de_plan():
    ...
    par_an = {}
    for l in lignes:
        par_an[l.annee] = par_an.get(l.annee, Decimal("0.00")) + l.montant
